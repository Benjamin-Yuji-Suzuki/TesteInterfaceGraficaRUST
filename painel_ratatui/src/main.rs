                ...
                match estado_clone {
                    ...
                    EstadoApp::Navegando => {
                        match key.code {
                            ...
                            KeyCode::Char('1') => { app.modo_atual = ModoVisao::Aulas; app.linha_selecionada = 0; app.table_state.select(Some(0)); }
                            KeyCode::Char('2') => { app.modo_atual = ModoVisao::Monitoria; app.linha_selecionada = 0; app.table_state.select(Some(0)); }
                            KeyCode::Char('3') => { app.modo_atual = ModoVisao::Intersecao; app.linha_selecionada = 0; app.table_state.select(Some(0)); }
                            
                            KeyCode::Up => {
                                app.linha_selecionada = app.linha_selecionada.saturating_sub(1);
                            ...
                            KeyCode::Char('a') | KeyCode::Char('A') => if app.modo_atual != ModoVisao::Intersecao {
                                app.estado = EstadoApp::EditandoHorario { novo: true };
                                app.input_atual = String::new();
                            },
                            KeyCode::Char('m') | KeyCode::Char('M') => if app.modo_atual != ModoVisao::Intersecao {
                                let grade = if app.modo_atual == ModoVisao::Aulas { &app.grade_aulas } else { &app.grade_monitorias };
                                if !grade.horarios.is_empty() {
                                    app.input_atual = grade.horarios[app.linha_selecionada].texto.clone();
                                    app.estado = EstadoApp::EditandoHorario { novo: false };
                                }
                            },
                            KeyCode::Enter => if app.modo_atual != ModoVisao::Intersecao {
                                let grade = if app.modo_atual == ModoVisao::Aulas { &app.grade_aulas } else { &app.grade_monitorias };
                            ...
                            KeyCode::Delete => if app.modo_atual != ModoVisao::Intersecao {
                                ...
                                if !grade.horarios.is_empty() && !grade.horarios[l].is_intervalo { grade.matriz[l][c] = Atividade::default(); }
                            },
                            _ => {}
                        }
                    }
                    EstadoApp::EditandoBloco => match key.code {
                        KeyCode::Enter => {
                            if app.modo_atual != ModoVisao::Intersecao {
                                ...
                                let grade = if app.modo_atual == ModoVisao::Aulas { &mut app.grade_aulas } else { &mut app.grade_monitorias };
                                grade.matriz[l][c] = Atividade { ativa: !input.is_empty(), descricao: input };
                            }
                    ...
                    EstadoApp::EditandoHorario { novo } => match key.code {
                    ...
                    EstadoApp::PerguntaIntervalo { texto_temp, novo } => match key.code {
                        KeyCode::Char('s') | KeyCode::Char('S') | KeyCode::Char('n') | KeyCode::Char('N') => {
                            ...
                            if app.modo_atual != ModoVisao::Intersecao {
                                let l = app.linha_selecionada;
                                let grade = if app.modo_atual == ModoVisao::Aulas { &mut app.grade_aulas } else { &mut app.grade_monitorias };
                                if novo { grade.horarios.push(Horario { texto: texto_temp, is_intervalo }); grade.matriz.push(vec![Atividade::default(); 5]); }
                                else { grade.horarios[l] = Horario { texto: texto_temp, is_intervalo }; if is_intervalo { grade.matriz[l] = vec![Atividade::default(); 5]; } }
                            }
                            app.estado = EstadoApp::Navegando;
                        }
                        KeyCode::Esc => app.estado = EstadoApp::Navegando,
