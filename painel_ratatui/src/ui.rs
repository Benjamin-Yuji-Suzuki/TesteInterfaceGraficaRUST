...
use crate::app::{App, Bloco, EstadoApp, ModoVisao};
...
// O motor de cores da célula
fn calcular_celula<'a>(bloco: &'a Bloco, modo: ModoVisao) -> Text<'a> {
    match modo {
        ModoVisao::Aulas => {
            if bloco.tem_aula { 
                Text::from(Span::styled(bloco.desc_aula.clone(), Style::default().fg(Color::Blue))) 
            } else { 
                Text::from(Span::styled("-", Style::default().fg(Color::DarkGray))) 
            }
        }
        ModoVisao::Monitoria => {
            if bloco.tem_monitoria { 
                Text::from(Span::styled(bloco.desc_monitoria.clone(), Style::default().fg(Color::Green))) 
            } else { 
                Text::from(Span::styled("-", Style::default().fg(Color::DarkGray))) 
            }
        }
        ModoVisao::Intersecao => {
            if bloco.tem_aula && bloco.tem_monitoria {
                let mut lines = vec![];
                lines.push(Line::from(vec![Span::styled("COLISÃO", Style::default().fg(Color::Rgb(255, 85, 0)).add_modifier(Modifier::BOLD))]));
                lines.push(Line::from(vec![
                    Span::styled(bloco.desc_aula.clone(), Style::default().fg(Color::Blue)),
                    Span::raw(" | "),
                    Span::styled(bloco.desc_monitoria.clone(), Style::default().fg(Color::Green)),
                ]));
                Text::from(lines)
...
pub fn desenhar_interface(frame: &mut Frame, app: &App) {
    ...
    let cabecalho = Paragraph::new(texto_cabecalho)
    ...
    let aba_selecionada = match app.modo_atual {
        ModoVisao::Aulas => 0, ModoVisao::Monitoria => 1, ModoVisao::Intersecao => 2,
    };

    let abas = Tabs::new(titulos_abas)
        .block(Block::default().borders(Borders::ALL).title(" Visão "))
        .select(aba_selecionada)
    ...
    for (i, horario) in app.horarios.iter().enumerate() {
        ...
        for dia_idx in 0..5 {
            ...
            if horario.is_intervalo {
                ...
                let bloco = &app.matriz[i][dia_idx];
                ...
                if dia_idx as u32 == app.dia_atual.num_days_from_monday() {
                    estilo_fundo = estilo_fundo.bg(Color::Rgb(30, 30, 30)); 
                }

                if matches!(app.estado, EstadoApp::Navegando | EstadoApp::EditandoBloco) {
        ...
        linhas_tabela.push(Row::new(celulas).height(2));
    }
    ...
    let texto_rodape = match app.estado {
        EstadoApp::Navegando => " [Setas] Navegar | [Enter] Editar Célula | [A] Novo Horário | [M] Modificar Horário | [Q] Sair",
    ...
    match &app.estado {
        EstadoApp::EditandoBloco => {
            let area_popup = area_centralizada(70, 20, area);
            frame.render_widget(Clear, area_popup); 
            let titulo = if app.modo_atual == ModoVisao::Aulas { " Editando Aula " } else { " Editando Monitoria " };
            let input = Paragraph::new(format!("{}_", app.input_atual))
                .style(Style::default().fg(Color::Green)).block(Block::default().borders(Borders::ALL).title(titulo));
            frame.render_widget(input, area_popup);
        }
        EstadoApp::EditandoHorario { novo } => {
            let area_popup = area_centralizada(50, 20, area);
            frame.render_widget(Clear, area_popup); 
            let titulo = if *novo { " Adicionar Horário " } else { " Modificar Horário " };
            let input = Paragraph::new(format!("{}_", app.input_atual))
                .style(Style::default().fg(Color::Cyan)).block(Block::default().borders(Borders::ALL).title(titulo));
            frame.render_widget(input, area_popup);
        }
        EstadoApp::PerguntaIntervalo { texto_temp, .. } => {
            let area_popup = area_centralizada(40, 20, area);
            frame.render_widget(Clear, area_popup); 
